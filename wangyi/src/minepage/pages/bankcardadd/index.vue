<template>
  <div class="account">
    ...
    <van-cell-group>
      ...
      <van-field :error='errorInfo.cardNum' @focus="onFocus('cardNum')" :value="cardNum" clearable required label="银行卡号"
        @change='changecardNum' placeholder="请输入银行卡号" />
      <van-field :error='errorInfo.cardName' @focus="onFocus('cardName')" :value="cardName" clearable required label="银行户名"
        @change='changecardName' placeholder="请输入银行户名" />
      <van-field :error='errorInfo.addressDetail' @focus="onFocus('addressDetail')" :value="addressDetail" label="选择地区"
        placeholder="选择省/市/区" readonly @click="showAreaPopup" required />
    </van-cell-group>
